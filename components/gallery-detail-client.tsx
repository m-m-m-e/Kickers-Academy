...
export function GalleryDetailClient({ slug }: { slug: string }) {
  ...
  const totalPages = Math.max(1, Math.ceil(activeItems.length / GALLERY_PAGE_SIZE));
  const visibleItems = activeItems.slice(page * GALLERY_PAGE_SIZE, page * GALLERY_PAGE_SIZE + GALLERY_PAGE_SIZE);
  ...
  const openFilteredItem = (index: number) => setSelectedIndex(page * GALLERY_PAGE_SIZE + index);
          ...
          <div className="mt-8">
            {activeItems.length ? (
              <>
                ...
                <div className="mt-8 grid gap-4 px-1 py-2 text-sm text-zinc-300 md:grid-cols-[1fr_auto_1fr] md:items-center">
